...
const CourseCard = ({course}) => {
  ...
  return (
    <Link to={"/course/"+course._id} onClick={()=>scrollTo(0,0)} className='border-2 border-gray-500/30 overflow-hidden rounded-lg pb-6 '>
      ...
      <div className='flex space-x-2 items-center'> <h3>4.5</h3> 
      <div className='flex space-x-2'>{[...Array(5)].map((_,i)=>(<img className='h-3.5 w-3.5' src={assets.star} alt='star' key={i}/>))}</div>
